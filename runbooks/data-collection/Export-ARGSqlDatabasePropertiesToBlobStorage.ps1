...
do
{
    ...
    {
        $dbs = (Search-AzGraph -Query $argQuery -First $ARGPageSize -Subscription $subscriptions).data
    }
    else
    {
        $dbs = (Search-AzGraph -Query $argQuery -First $ARGPageSize -Skip $resultsSoFar -Subscription $subscriptions).data
    }
